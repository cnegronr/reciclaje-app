    ...
    if (backendRecord) {
      ...
      (backendRecord.detalles || []).forEach((d) => {
        ...
        detallesMap[contId] = {
          ...
          porcentajeEstimado: Number(d.porcentajeEstimado || 0),
          kilosCalculados: Number(d.kilosCalculados || 0),
          porcentajeEstimadoInicial: Number(d.porcentajeEstimadoInicial ?? d.porcentajeEstimado ?? 0),
          kilosCalculadosInicial: Number(d.kilosCalculadosInicial ?? d.kilosCalculados ?? 0),
          visitado: Boolean(d.visitado),
          fechaHoraInicial: d.fechaHoraInicial,
          fechaHoraActualizacion: d.fechaHoraActualizacion,
          observaciones: d.observaciones || '',
          observacionesInicial: d.observacionesInicial ?? d.observaciones ?? '',
          fotosInicialesAntes,
  ...
  saveDetalleInspeccion: async (comunaId, inspectorId, contenedorId, inspectionData, isUpdateMode = false, backendComunaId = null) => {
    ...
    if (!isUpdateMode) {
      record.detalles[contenedorId] = {
        ...
        porcentajeEstimadoInicial: inspectionData.porcentajeEstimado,
        kilosCalculadosInicial: inspectionData.kilosCalculados,
        observaciones: inspectionData.observaciones || '',
        observacionesInicial: inspectionData.observaciones || '',
